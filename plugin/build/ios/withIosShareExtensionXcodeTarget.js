...
const withShareExtensionXcodeTarget = (config, parameters) => {
    return (0, config_plugins_1.withXcodeProject)(config, async (config) => {
        ...
        pbxProject.addSourceFile(toRelativePath((0, writeIosShareExtensionFiles_1.getShareExtensionViewControllerPath)(platformProjectRoot, parameters)), { target: target.uuid }, pbxGroupKey);
        // Add the resource file and include it into the target PbxResourcesBuildPhase and PbxGroup
        try {
            // ShareExtensionPreprocessor.js
            pbxProject.addResourceFile(toRelativePath((0, writeIosShareExtensionFiles_1.getPreprocessorFilePath)(platformProjectRoot, parameters)), { target: target.uuid }, pbxGroupKey);
            // MainInterface.storyboard
            pbxProject.addResourceFile(toRelativePath((0, writeIosShareExtensionFiles_1.getShareExtensionStoryboardFilePath)(platformProjectRoot, parameters)), { target: target.uuid }, pbxGroupKey);
            // PrivacyInfo.xcprivacy
            pbxProject.addResourceFile(toRelativePath((0, writeIosShareExtensionFiles_1.getPrivacyInfoFilePath)(platformProjectRoot, parameters)), { target: target.uuid }, pbxGroupKey);
        }
        catch (e) {
