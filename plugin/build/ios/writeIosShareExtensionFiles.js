...
function getPreprocessorContent(parameters) {
    ...
    if (preprocessorFunctionFile) {
        preprocessorFunction = node_fs_1.default.readFileSync(preprocessorFunctionFile, "utf8");
    }
    const injection = parameters.preprocessorInjectJS || "";
    if (preprocessorFunction && injection) {
        throw new Error("[expo-share-intent] preprocessorFunctionJS and preprocessorInjectJS are mutually exclusive");
    }
...
function getShareExtensionViewControllerContent(scheme, groupIdentifier) {
    ...
    console.debug(`[expo-share-intent] add ios share extension (scheme:${updatedScheme} groupIdentifier:${groupIdentifier})`);
    if (!updatedScheme) {
